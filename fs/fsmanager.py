    ...
    def analyse_data(self):
        ...
        for i in range(0, self._nb_file):
            ...
            if self._fi_data_list[i].compression_used:
                self._data_list.append(self.lzs.decode(self._fs_data[start_data:end_data]))
            else:
                self._data_list.append(self._fs_data[start_data:end_data])

    def get_generator_fs_data_analyzed(self) -> list[Generator[int, None, None]]:
        return self._data_list

    def get_fs_data_analyzed(self) -> list[bytes]:
        print("get_fs_data_analyzed_computed")
        data_byte = []
        for data in self._data_list:
            data_byte.append(bytes(data))
            print(f"len data: {len(data_byte[-1])}")
        print(f"len: {len(data_byte)}")
        return data_byte
...
if __name__ == "__main__":
    ...
    archive_fs_data = archive_magic.get_generator_fs_data_analyzed()
    ...
    # Fast method to read hex (as nothing is loaded till needed)
    #print(f"Fs hex: {bytes(archive_fs_data[0]).hex(" ")}")
    # Slow method to read hex (as everything is loaded in memory ready to be used)
    #archive_fs_data_bytes = archive_magic.get_fs_data_analyzed()
    #print(f"Fs hex2: {archive_fs_data_bytes[0].hex(" ")}")
