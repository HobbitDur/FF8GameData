class Lzs:
    ...
    @staticmethod
    def encode(input_bytes: bytearray) -> bytearray:
        context = Lzs.EncodeContext()
        return context.encode(input_bytes)

    @staticmethod
    def decode(input_bytes: bytearray) -> bytearray:
        context = Lzs.EncodeContext()
        return context.decode(input_bytes)
    ...
    class EncodeContext:
        ...
        def decode(self, input_bytes: bytearray) -> bytearray:
            output_bytes = bytearray()
            self.init_tree()
            flags = 0
            input_pos = 0
            ...
            while input_pos < len(input_bytes):
                ...
                if flags & 1:
                    if input_pos < len(input_bytes):
                        output_bytes.append(input_bytes[input_pos])
                        self.buffer[r] = input_bytes[input_pos]
                        input_pos += 1
                        r = (r + 1) & (Lzs.N - 1)
                    else:
                        break
